...
[JSSDocs](https://jsdoc.app/) is a way for us to document our code. It provides three major benefits:

1. It gives developers information about your code when they are using it
2. It converts automatically your code to TypeScript
...
We are going to add a function description between the `/**` and `**/` comment tags. We are first going to do this for a normal JS function and then we are going to do it for a React component. You can use JSDocs for both normal JS function as well as React components.
...
![JSDocs Tooltip](./jsdocs-image-0.png)
...
Note: TypeScript is able to automatically add types to your code if JSDocs has been added. If there is JSDocs in a TypeScript file and typing hasn't been done then you will see a white squiggle line under the function name which says `JSDoc types may be moved to TypeScript types` in the tool tip. In Quick Fix is the option `Annotate with type from JSDoc` which will automatically convert the function for you.
...
We begin by adding the `@example` property. We then add a Markdown codeblock with JavaScript Syntax highlighting (`js`). We show how to use the function so that others can easily use the function or React component themselves without looking into the code.
...
1. The data types at the top (`function addNumbers(a: number, b: number): number`), this lets a developer quickly see what data types they need to be passing in.
...
![JSDocs @example Tooltip](./jsdocs-image-1.png)
...
You can create a `@typedef` type and use that as your `@param`. See the example showing in the `@typedef` section above.

## Generating documentation:

One of the benefits to JSDocs is that it can generate documentation. This documentation is in HTML and is already styled for you.
...
![JSDocs Generated HTML](./jsdocs-image-2.png)
...
The end result is that everything will be use, excluding the `node_modules` folder.
